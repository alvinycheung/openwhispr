...
- **meetingTranscriptionRouting.js**: Renderer-side resolver from Note Recording settings to `meeting-transcription-start` options. `self-hosted` with a URL yields `provider: "self-hosted"`; the main process resolves it through `resolveTranscriptionRoute` at start and posts each 5s chunk to the OpenAI-compatible `/audio/transcriptions` endpoint on the same chunk loop local mode uses. Without a URL it throws the `unsupportedSelfHosted` sentinel
...
- **nemoSpeechDiarizer.js**: Pure pieces of the optional `nemo-speech` diarization engine (NVIDIA Nemotron 3 Diarization through NeMo-Speech.cpp): binary probe paths, `diarize` arguments, the `/v1/audio/diarizations` URL builder, and the RTTM and JSON parsers that renumber speakers to match sherpa-onnx. `diarization.js` picks the engine from `DIARIZATION_ENGINE`, spawns whichever binary applies, and falls back to the bundled sherpa-onnx pipeline when nemo-speech is missing or fails. Unit-tested in `test/helpers/nemoSpeechDiarizer.test.js`
...
- `DIARIZATION_SERVER_URL`: a `nemo-speech serve` base URL (`http://host:8100`). With the nemo-speech engine, the wav is posted to `/v1/audio/diarizations` there first; a local nemo-speech binary and then sherpa-onnx are the fallbacks
